...
`master` is the semantic-release branch. Release configuration is embedded in `package.json`; there is no separate `release.config.mjs`.

`.github/workflows/release.yml` installs dependencies with Bun, runs type checking and unit tests, then runs `bun run release`. semantic-release calculates the version from Conventional Commits, publishes to npm, creates the Git tag and GitHub release, and generates release notes.

npm publishing uses GitHub Actions OIDC trusted publishing. Configure the npm Trusted Publisher with:

- Owner: `JasonLandbridge`
- Repository: `pi-omniroute-sync`
- Workflow filename: `release.yml`

The release job requires `contents: write`, `issues: write`, `pull-requests: write`, and `id-token: write`. It does not require a long-lived `NPM_TOKEN` after trusted publishing is configured.
